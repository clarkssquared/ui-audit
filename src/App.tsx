...
interface Issue {
  title: string; severity: string; wcagCriterion: string; description: string;
  recommendation: string; prometSourceSolution: string; visualLocation: string;
  elementHtml: string; cssSelector: string; devtoolsSnippet: string;
}
...
const sevStyle = (s: string): SevStyle => SEV[s] || { color:'#374151', bg:'#f3f4f6', border:'#9ca3af', icon:null, label: s+' severity' };
...
const globalStyles = `
  ...
  *:focus-visible {
  ...
  @media (min-width: 576px) {
    .audit-container { padding-left: 24px; padding-right: 24px; }
  }
  @media (min-width: 768px) {
    .audit-container { padding-left: 32px; padding-right: 32px; }
  }
  @media (min-width: 992px) {
    .audit-container { padding-left: 48px; padding-right: 48px; }
  }
  @media (min-width: 1400px) {
    .audit-container { padding-left: 64px; padding-right: 64px; }
  }

  /* Responsive grid for audit types */
  .audit-type-grid {
    display: grid;
  ...
  /* Responsive category score grid */
  .cat-score-grid {
    display: grid;
    grid-template-columns: 1fr;
    gap: 8px;
  ...
  @media (min-width: 576px) {
    .cat-score-grid { grid-template-columns: 1fr 1fr; }
  }
  @media (min-width: 992px) {
    .cat-score-grid { grid-template-columns: repeat(3, 1fr); }
  }

  /* Responsive severity summary grid */
  .severity-grid {
    display: grid;
  ...
  @media (min-width: 576px) {
    .summary-header { flex-direction: row; align-items: center; justify-content: space-between; }
  }

  /* Responsive page result header */
  .page-result-header {
    display: flex;
  ...
  @media (min-width: 576px) {
    .page-result-header { gap: 16px; align-items: center; }
  }
  ...
  @media (min-width: 768px) { .result-card-body { padding: 20px 28px; } }
`;
      ...
      <h4 style={{margin:0}}>
        <button id={hId} onClick={()=>setOpen(o=>!o)} aria-expanded={open} aria-controls={rId}
          ...
          <div style={{display:'flex',alignItems:'center',gap:12}}>
            <div style={{width:44,height:44,borderRadius:'50%',border:`3px solid ${col}`,display:'flex',flexShrink:0,
          ...
          <span aria-hidden="true" style={{color:'#6b7280',flexShrink:0,marginLeft:8}}>{open?<ChevronUp size={18}/>:<ChevronDown size={18}/>}</span>
        </button>
      </h4>
            ...
            return (
              <article key={i} aria-labelledby={iId}
                ...
                <div style={{display:'flex',alignItems:'flex-start',justifyContent:'space-between',gap:10,marginBottom:8,flexWrap:'wrap'}}>
                  <h5 id={iId} style={{fontWeight:600,fontSize:14,color:DARK,margin:0,flex:1,minWidth:0}}>{iss.title}</h5>
    ...
    <section aria-labelledby={hId} style={{background:'#fff',borderRadius:14,boxShadow:'0 2px 16px rgba(0,0,0,.08)',marginBottom:20,overflow:'hidden'}}>
      <h3 style={{margin:0}}>
        <button id={hId} onClick={()=>setOpen(o=>!o)} aria-expanded={open} aria-controls={rId}
          ...
          <div className="page-result-header" style={{flex:1,minWidth:0}}>
            <ScoreRing score={pr.result?.overallScore||0} size={56} id={`ring-${index}`}/>
            <div style={{textAlign:'left',minWidth:0,flex:1}}>
              <span style={{fontSize:12,fontWeight:700,color:ORANGE,textTransform:'uppercase',letterSpacing:'.07em',display:'block',marginBottom:2}}>Page {index+1}</span>
              <span style={{fontWeight:700,fontSize:14,color:DARK,display:'block',marginBottom:4,wordBreak:'break-all'}}>{pr.url}</span>
          ...
          <span aria-hidden="true" style={{color:'#6b7280',flexShrink:0,marginLeft:8}}>{open?<ChevronUp size={20}/>:<ChevronDown size={20}/>}</span>
        </button>
      </h3>
      <div id={rId} role="region" aria-labelledby={hId} hidden={!open}>
          ...
          <div className="cat-score-grid">
            {pr.result?.categories?.map((cat: Category, i: number) => (
              <div key={i} style={{display:'flex',justifyContent:'space-between',alignItems:'center',background:LIGHT,borderRadius:8,padding:'8px 12px'}}>
                ...
                <span style={{fontSize:13,fontWeight:700,color:scoreColor(cat.score),flexShrink:0,marginLeft:8}}>{cat.score}/100 <span className="sr-only">({scoreLabel(cat.score)})</span></span>
              </div>
            ))}
          </div>
...
export default function UIAuditTool() {
  ...
  useEffect(() => {
    if (statusMsg) { const t = setTimeout(() => setStatusMsg(''), 6000); return () => clearTimeout(t); }
  }, [statusMsg]);
...
- Do not truncate — the JSON must be complete and valid`;
  ...
  const runAudit = async () => {
    ...
    for (let i = 0; i < validUrls.length; i++) {
      ...
      try {
        ...
        const msg = raw.includes('RATE_LIMIT') ? 'Rate limit reached. Please wait 10–15 minutes.' : raw;
  ...
  const summary = pageResults.length > 0 ? (() => {
    ...
    ok.forEach(p => p.result?.categories?.forEach((c:Category) => c.issues?.forEach((iss:Issue) => { counts.total++; if (counts[iss.severity]!==undefined) counts[iss.severity]++; })));
  ...
  const copyCSV = () => {
    const fb = () => { const ta=document.createElement('textarea'); ta.value=csvData; ta.style.cssText='position:fixed;left:-9999px'; document.body.appendChild(ta); ta.select(); try { document.execCommand('copy'); setCopySuccess(true); setStatusMsg('Copied.'); setTimeout(()=>setCopySuccess(false),3000); } catch { /* silent */ } document.body.removeChild(ta); };
    if (navigator.clipboard) { navigator.clipboard.writeText(csvData).then(()=>{setCopySuccess(true);setStatusMsg('Copied.');setTimeout(()=>setCopySuccess(false),3000);}).catch(fb); } else fb();
  };
  ...
  return (
    <>
      ...
      <div style={{minHeight:'100vh',width:'100%',background:LIGHT,fontFamily:"'Segoe UI',system-ui,-apple-system,sans-serif"}}>

        {/* Header */}
        <header role="banner" style={{background:NAVY,width:'100%',position:'sticky',top:0,zIndex:100,boxShadow:'0 2px 12px rgba(0,0,0,.3)'}}>
          <div className="audit-container" style={{display:'flex',alignItems:'center',justifyContent:'space-between',height:64}}>
            <div style={{display:'flex',alignItems:'center',gap:12}}>
              <div style={{width:36,height:36,background:ORANGE,borderRadius:6,display:'flex',alignItems:'center',justifyContent:'center',fontWeight:900,color:'#fff',fontSize:16,flexShrink:0}} aria-hidden="true">P</div>
              <div>
                <span style={{color:'#fff',fontWeight:700,fontSize:15,display:'block'}}>Promet Source</span>
                <span style={{color:'#94a3b8',fontSize:11,letterSpacing:'.05em',textTransform:'uppercase'}}>Audit Tool</span>
              </div>
            </div>
            <a href="https://www.prometsource.com/contact" target="_blank" rel="noopener noreferrer"
              style={{background:ORANGE,color:'#fff',fontWeight:700,fontSize:13,padding:'8px 20px',borderRadius:6,textDecoration:'none',whiteSpace:'nowrap'}}>
        ...
        {/* Hero */}
        <div style={{background:`linear-gradient(135deg,${NAVY} 0%,#243352 100%)`,width:'100%',padding:'48px 0'}}>
          <div className="audit-container" style={{textAlign:'center'}}>
            ...
            <h1 className="hero-title" style={{color:'#fff',fontWeight:800,marginBottom:14,lineHeight:1.15,letterSpacing:'-.02em'}}>
        ...
        {/* Main */}
        <main id="main-content" tabIndex={-1} style={{width:'100%'}}>
          <div className="audit-container" style={{paddingTop:0,paddingBottom:48}}>

            {/* Form card */}
            <section aria-labelledby="form-heading"
              style={{background:'#fff',borderRadius:14,boxShadow:'0 4px 24px rgba(0,0,0,.1)',position:'relative',zIndex:10,marginBottom:24,marginTop:'-28px'}}>
              <div className="form-card">
                <h2 id="form-heading" style={{fontSize:18,fontWeight:700,color:DARK,marginBottom:20}}>Configure Your Audit</h2>
                    ...
                    <div key={i} style={{display:'flex',gap:8,marginBottom:10,alignItems:'center'}}>
                      ...
                      {urls.length > 1 && (
                        <button onClick={() => removeUrl(i)} aria-label={`Remove page ${i+1}`}
                          style={{background:'#fef2f2',border:'2px solid #fecaca',borderRadius:8,padding:'10px',cursor:'pointer',color:'#b91c1c',display:'flex',alignItems:'center',flexShrink:0}}>
                      ...
                      return (
                        <label key={o.val} style={{display:'flex',alignItems:'center',gap:12,padding:'13px 16px',
                          ...
                          <input type="radio" name="auditType" value={o.val} checked={active}
                            onChange={() => setAuditType(o.val)} style={{accentColor:ORANGE,width:16,height:16,flexShrink:0}}/>
                          <div>
                  ...
                  {loading && (
                    <button onClick={stopAudit}
                      style={{background:'#fef2f2',border:'2px solid #ef4444',color:'#b91c1c',
                        fontWeight:700,fontSize:14,padding:'14px 20px',borderRadius:10,cursor:'pointer',flexShrink:0}}>
                      Stop
                    </button>
                  )}
                </div>
            ...
            {pageResults.length > 0 && (
              <div ref={resultsRef} tabIndex={-1}>
                {summary && (
                  <section aria-labelledby="summary-h"
                    ...
                    <div className="summary-header">
                      <div>
                        <p style={{fontSize:12,fontWeight:700,color:ORANGE,textTransform:'uppercase',letterSpacing:'.08em',marginBottom:6}}>Consolidated Report</p>
                      ...
                      <div style={{textAlign:'center',flexShrink:0}}>
                        ...
                        <p style={{fontSize:12,color:'#6b7280',marginTop:4,fontWeight:600}}>Avg. Score</p>
                      </div>
                    </div>
                    ...
                    {copySuccess && (
                      <div role="status" style={{marginTop:12,background:'#f0fdf4',border:'2px solid #16a34a',borderRadius:10,padding:'14px 18px'}}>
                        <p style={{fontWeight:700,color:'#14532d',marginBottom:4}}><CheckCircle size={16} style={{verticalAlign:'middle',marginRight:6}} aria-hidden="true"/>CSV Downloaded</p>
                    ...
                    {showCsvText && (
                      <div style={{marginTop:12,background:'#eff6ff',border:'2px solid #2563eb',borderRadius:10,padding:18}}>
                        <h3 style={{fontWeight:700,color:'#1d4ed8',marginBottom:10,fontSize:15}}>Copy &amp; Paste to Google Sheets</h3>
                        ...
                        <ol style={{fontSize:13,color:'#1e40af',paddingLeft:20,marginBottom:10}}>
                ...
                <h2 style={{fontSize:14,fontWeight:700,color:'#6b7280',textTransform:'uppercase',letterSpacing:'.07em',marginBottom:12}}>Page-by-Page Results</h2>
                ...
                {pageResults.map((pr,i) =>
                  pr.error ? (
                    <section key={i} aria-labelledby={`err-${i}`} role="alert"
                      ...
                      <h3 id={`err-${i}`} style={{fontWeight:700,color:DARK,marginBottom:6,fontSize:15}}>Page {i+1}: {pr.url}</h3>
                ...
                <div style={{background:`linear-gradient(135deg,${NAVY} 0%,#243352 100%)`,borderRadius:14,padding:'40px 32px',textAlign:'center',marginTop:8}}>
                  <h2 style={{color:'#fff',fontSize:22,fontWeight:800,marginBottom:10}}>Ready to fix these issues?</h2>
                  <p style={{color:'#94a3b8',fontSize:15,marginBottom:24,lineHeight:1.6}}>Promet Source specializes in accessibility, performance, and migrations for the public sector.</p>
                  <a href="https://www.prometsource.com/contact" target="_blank" rel="noopener noreferrer"
