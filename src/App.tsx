...
const scoreColor = (s) =>
  s >= 80 ? '#15803d' : s >= 60 ? '#a16207' : '#b91c1c';
const scoreLabel = (s) =>
  s >= 80 ? 'Good' : s >= 60 ? 'Needs improvement' : 'Critical';
...
const extractJSON = (raw) => {
  ...
  const tries = [
    ...
    () =>
      JSON.parse(
        json.replace(/[\x00-\x1F\x7F]/g, ' ').replace(/,(\s*[}\]])/g, '$1')
      ),
  ];
...
const ScoreRing = ({ score, size = 80, id }) => {
  ...
  return (
    ...
    >
      <title id={id + '-d'}>
        Score {score} out of 100 — {scoreLabel(score)}
      </title>
      <circle
        cx="40"
        cy="40"
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            ...
            <div style={{ textAlign: 'left' }}>
              ...
              >
                {cat.issues?.length || 0} finding
                {cat.issues?.length !== 1 ? 's' : ''} · {cat.score}/100 (
                {scoreLabel(cat.score)})
              </span>
            </div>
          </div>
          <span aria-hidden="true" style={{ color: '#6b7280', flexShrink: 0 }}>
              ...
              return (
                ...
                >
                  ...
                  {iss.wcagCriterion && iss.wcagCriterion !== 'N/A' && (
                    ...
                    >
                      <strong>WCAG:</strong> {iss.wcagCriterion}
                    </p>
                  )}
                  <p
                    style={{
                  ...
                  {iss.prometSourceSolution && (
                    <div
                      role="note"
                      aria-label="Promet Source solution"
                      style={{
                        background: '#fff7f3',
...
const PageResultCard = ({ pr, index }) => {
  ...
  const counts = (() => {
    ...
    pr.result?.categories?.forEach((cat) =>
      cat.issues?.forEach((iss) => {
        c.total++;
        if (c[iss.severity] !== undefined) c[iss.severity]++;
      })
    );
    return c;
  })();
      ...
      <h3 style={{ margin: 0 }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
            <ScoreRing
              score={pr.result?.overallScore || 0}
              size={64}
              id={`ring-${index}`}
            />
            <div style={{ textAlign: 'left' }}>
              ...
              >
                {[
                  ...
                  ([s, c]) =>
                    counts[s] > 0 && (
                      <span key={s} style={{ fontWeight: 600, color: c }}>
                        {counts[s]} {s}
                      </span>
                    )
                )}
          ...
          >
            {pr.result?.categories?.map((cat, i) => (
              ...
              >
                ...
                >
                  {cat.score}/100{' '}
                  <span className="sr-only">({scoreLabel(cat.score)})</span>
                </span>
              </div>
            ))}
          </div>
          {pr.result?.categories?.map((cat, i) => (
            <CategoryCard key={i} cat={cat} ci={i} pi={index} />
          ))}
        </div>
      </div>
    </section>
...
export default function UIAuditTool() {
  ...
  const catList = {
    ...
    accessibility:
      'Perceivable, Operable, Understandable, Robust, SEO Basics, Performance Indicators',
    migration:
      'Content Inventory, File Assets, Technical Architecture, Data Structures, Integrations, SEO Basics, Performance Indicators',
    technical:
      'SEO Fundamentals, Performance Analysis, Code Quality, Security Basics, Best Practices',
  };

  const buildPrompt = (
    url
  ) => `You are an expert web auditor. Search for and analyze this page: ${url}
...
- Do not truncate — the JSON must be complete and valid`;
    ...
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        if (err.message.includes('RATE_LIMIT') && attempt < retries) {
          ...
          setStatusMsg(
            `Rate limit hit — waiting ${wait / 1000}s before retrying…`
          );
          await sleep(wait);
        } else {
          throw err;
    ...
    while (attempts < MAX_TURNS) {
      ...
      if (fullText.trim()) {
        try {
          ...
          if (result.overallScore !== undefined && result.categories)
            return result;
        } catch {}
      }
      ...
      if (data.stop_reason === 'end_turn') {
        messages = [
          ...
          { role: 'assistant', content },
          {
            role: 'user',
            content:
              'Now provide ONLY the raw JSON audit report. Start immediately with { and nothing else.',
          },
        ];
        continue;
      }
      ...
      if (data.stop_reason === 'tool_use') {
        ...
        const toolResults = toolUseBlocks.map((b) => ({
          type: 'tool_result',
          tool_use_id: b.id,
          content: `Search completed for: ${JSON.stringify(
            b.input
          )}. Now generate the complete JSON audit report.`,
        }));
        messages = [
          ...messages,
          { role: 'assistant', content },
      ...
      messages = [
        ...
        { role: 'assistant', content },
        {
          role: 'user',
          content:
            'Provide the JSON audit report now. Start with { and end with }. No other text.',
        },
      ];
    }
  ...
  const runAudit = async () => {
    ...
    if (bad) {
      setError(`"${bad}" must start with http:// or https://`);
      return;
    }
    ...
    setStatusMsg(
      `Audit started for ${validUrls.length} page${
    ...
    for (let i = 0; i < validUrls.length; i++) {
      ...
      try {
        ...
        setStatusMsg(
          `Page ${i + 1} complete. Score: ${result.overallScore}/100.`
        );
      } catch (err) {
      ...
      if (i < validUrls.length - 1) {
        ...
        setStatusMsg(
          `Cooling down before next page (${Math.round(delay / 1000)}s)…`
        );
    ...
    setLoading(false);
    setStatusMsg(
      `Audit complete. ${results.filter((r) => r.result).length} of ${
        validUrls.length
      } analyzed.`
    );
  ...
  const summary =
    pageResults.length > 0
      ? (() => {
          ...
          const avg = ok.length
            ? Math.round(
                ok.reduce((a, p) => a + p.result.overallScore, 0) / ok.length
              )
            : 0;
          let counts = { Critical: 0, High: 0, Medium: 0, Low: 0, total: 0 };
          ok.forEach((p) =>
            p.result.categories?.forEach((c) =>
          ...
          return {
            ...
            counts,
            ok: ok.length,
            failed: pageResults.filter((p) => p.error).length,
          };
        })()
      : null;
  ...
  const exportCSV = () => {
    const rows = [];
    rows.push(
      ['PROMET SOURCE – MULTI-PAGE AUDIT REPORT'],
      [''],
      [
    ...
    pageResults.forEach((pr, pi) => {
      ...
      rows.push(
        [`Score: ${pr.result.overallScore}/100`],
        [pr.result.summary || ''],
        [[]]
      );
    ...
    try {
      ...
      a.download = `Promet-Audit-${Date.now()}.csv`;
    ...
    <>
      ...
      >
        ...
        >
          ...
          <a
            href="https://www.prometsource.com/contact"
            target="_blank"
        ...
        <div
          style={{
            background: `linear-gradient(135deg,${NAVY} 0%,#243352 100%)`,
            padding: '56px 48px',
          }}
        >
          <div
          ...
          >
            <p
              style={{
                ...
                background: 'rgba(201,90,31,.15)',
                border: '1px solid rgba(201,90,31,.4)',
                borderRadius: 20,
                padding: '4px 16px',
                fontSize: 12,
                color: '#f97316',
            ...
            >
              AI-Powered · Multi-Page · WCAG 2.2 AA
            </p>
            <h1
              style={{
                color: '#fff',
            ...
            >
              UI &amp; Accessibility
              <br />
              <span style={{ color: '#f97316' }}>Audit Tool</span>
            </h1>
            <p
              style={{
                color: '#94a3b8',
            ...
            >
              AI-powered analysis with pinpoint issue location.
              <br />
              Built to WCAG 2.2 AA — accessible to every member of your team.
            </p>
          </div>
        </div>
        ...
        >
          ...
          >
            ...
            >
                ...
                >
                  <div style={{ position: 'relative', flex: 1 }}>
                    <Search
                      ...
                      aria-hidden="true"
                    />
                    <label htmlFor={`url-${i}`} className="sr-only">
                      Page {i + 1} URL
                    </label>
                    <input
                      id={`url-${i}`}
                      ...
                      aria-describedby={
                        error && i === 0 ? 'url-error' : undefined
                      }
                      style={{
                        width: '100%',
                        padding: '12px 12px 12px 40px',
                        border: `2px solid ${u ? ORANGE : '#d1d5db'}`,
                        borderRadius: 8,
                        fontSize: 14,
                        outline: 'none',
              ...
              <button
                onClick={addUrl}
                disabled={urls.length >= 7}
                aria-label={
                  urls.length >= 7
                    ? 'Maximum 7 pages reached'
                ...
                style={{
                  ...
                  color: urls.length >= 7 ? '#9ca3af' : ORANGE,
                  fontWeight: 600,
                  fontSize: 13,
                  padding: '9px 18px',
                  borderRadius: 8,
                  cursor: urls.length >= 7 ? 'not-allowed' : 'pointer',
                  marginTop: 4,
                  transition: 'all .2s',
                }}
              >
                ...
                {[
                  {
                    val: 'full',
                    label: 'Full UI Audit',
                    sub: 'UX + SEO + Performance',
                  },
                  {
                    val: 'accessibility',
                    label: 'Accessibility Focus',
                    sub: 'WCAG 2.2 + SEO + Speed',
                  },
                  {
                    val: 'migration',
                    label: 'Migration Assessment',
                    sub: 'Content + Files + Tech',
                  },
                  {
                    val: 'technical',
                    label: 'Technical Audit',
                    sub: 'SEO + Performance + Security',
                  },
                ].map((o) => {
            ...
            <div style={{ display: 'flex', gap: 10 }}>
              <button
                ...
                style={{
                  ...
                  justifyContent: 'center',
                  gap: 10,
                  boxShadow: loading ? 'none' : `0 4px 14px rgba(201,90,31,.4)`,
                  transition: 'all .2s',
                }}
              >
                {loading ? (
                  ...
                  <>
                    ...
                    Run Audit ({validCount} page{validCount !== 1 ? 's' : ''})
                  </>
                )}
              </button>
              {loading && (
          ...
          {pageResults.length > 0 && (
            <div ref={resultsRef} tabIndex={-1}>
                        ...
                        >
                          ...
                          >
                            {pr.result ? (
                              <span
                                style={{
                                  color: scoreColor(pr.result.overallScore),
                                }}
                              >
                                {pr.result.overallScore}/100
                              </span>
                            ) : (
                  ...
                  {copySuccess && (
                    ...
                    >
                      ...
                      <p style={{ fontSize: 13, color: '#374151', margin: 0 }}>
                        Open in Google Sheets via File → Import → Upload.
                      </p>
                    </div>
                  )}
                  {showCsvText && (
              ...
              >
                ...
                >
                  Promet Source specializes in accessibility, performance, and
                  migrations for the public sector.
                </p>
                <a
                  href="https://www.prometsource.com/contact"
                  target="_blank"
                  ...
                  style={{
                    ...
                    borderRadius: 10,
                    textDecoration: 'none',
                    boxShadow: '0 4px 14px rgba(201,90,31,.5)',
                  }}
                >
